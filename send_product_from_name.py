...
def send_product_from_name(spfnArr):
    print("----send_product_from_name----")

    print("spfnArr: ", spfnArr)
    prod_id = spfnArr.pop(0)
    print("prod_id: ", prod_id)

    query = 'select destination_str, address, post_proc from ' + f"generic_distribution where product_name = '{prod_id}' and enabled"
    print('select destination_str, address, post_proc from ' + f"generic_distribution where product_name = '{prod_id}' and enabled")
