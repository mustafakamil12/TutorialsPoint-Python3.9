...
def ParseWXA_XML(site_id,data_response):
    ...
    myhour = root.findall("./WeatherData/Hours/Hour")
    myelem = root.findall("./WeatherData/Hours/Hour//")
    mysubnodes = root.findall("./WeatherData/Hours/Hour/")

    for node in mysubnodes:
        print(f"node.tag = {node.tag} , node.attrib = {node.attrib}")


"""
    for hour in myhour:
        print(f"hour.tag = {hour.tag} , hour.attrib = {hour.attrib}")
...
call = "http://api.weatheranalytics.com/v2/wsi/metar/[37.3167,-79.9667]?startDate=06/28/2021&endDate=06/30/2021&interval=hourly&units=imperial&format=xml&userKey=4449f2f578c59aa4eb638a9c5d39ec52&time=lwt&limit=25"
